...
def get_detail(place_id):
    ...
    if "current_popularity" in detail_json:
...
def run(_params):
    ...
    print("Finished in: {}".format(str(datetime.datetime.now() - start)))
    ...
    # pool.map(worker_detail, g_places)
    # pool.close()
    # pool.join()
    for i in range(params["n_threads"]):
